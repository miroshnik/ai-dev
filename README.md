...
- `github` — проект GitHub репозитория по канону: `project check` сверяет
  по пунктам ✅/❌, `project fix` доводит до канона (копия эталона, API,
  шаги UI со ссылками, удаление — с подтверждением).
