...
`est history --all-repos`, затем `--analogs owner/repo#254,owner/repo#260`
(репо должен быть в реестре); в комментарии они подписаны «из проекта …»,
в доверие входят как B, в k текущего репо — нет. Фактов нет нигде →
...
`est.py`, дата там же; запись кэша = вход ×1,25 для TTL 5 мин и ×2 для 1 ч;
...
`~/.claude/est/prices.json`: `{"<префикс id модели>": [вход, выход, чтение_кэша]}`;
...
- сессия названа `#<номер> <название задачи>` — название читается из
  транскрипта и привязывает сессию к задаче явно (сильнее первого промпта);
