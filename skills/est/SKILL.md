...
- Облачная сессия Claude Code (`CLAUDE_CODE_REMOTE=true`): GraphQL и проекты
  GitHub там закрыты, а её транскрипт локальному `est` не виден. `est fact <N>`
  в облаке печатает комментарий «Факт недоступен (облако)» со ссылкой на
  сессию — записать его в issue инструментом GitHub; остальные команды —
  ошибка с объяснением (`docs/cloud-sessions.md` в ai-dev). Задачу, начатую
  в облаке и доведённую локально, факт видит только локальной частью —
  строка «Причина: пропущена работа: …» со ссылкой на облачную сессию.
