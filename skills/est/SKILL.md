...
allowed-tools: Bash(python3 *skills/est/scripts/est.py *) Bash(gh issue view *) Bash(gh pr view *)
---
...
Codex), а не сообщается по памяти. Скрипт `scripts/est.py` делает всё детерминированное
...
Запуск: `python3 <каталог скилла>/scripts/est.py <подкоманда> …` — каталог
