...
Транскрипт облачной сессии Claude Code (claude.ai/code) лежит в облаке, и
факт видит её работу, только если её события импортированы. Коммиты облачной
сессии несут трейлер `Claude-Session: https://claude.ai/code/session_…`: PR с
таким коммитом без импорта даёт покрытие `partial` и строку «Облачная сессия
… не импортирована — est cloud-import» в комментарии «Факт».
...
- Облачная сессия Claude Code (`CLAUDE_CODE_REMOTE=true`): GraphQL и проекты
  GitHub там закрыты. `est fact <N>` в облаке печатает комментарий «Факт
  недоступен (облако)» со ссылкой на сессию — записать его в issue
  инструментом GitHub; остальные команды — ошибка с объяснением
  (`docs/cloud-sessions.md` в ai-dev). Факт доводит локальная сессия:
  импорт событий облачной сессии (раздел «Облачная сессия») и
  `est fact <N> --write`.
