...
- `README.md` — индекс: «Что делает система» (по capability), «Как построена»
  (по стандарту); у каждой ссылки — первый абзац шапки главного файла
  (описание) и, если есть, число пропущенных и падающих; раздел «Вне дерева» —
  только если такие тесты есть;
- `capabilities/<name>.md`, `standards/<name>.md` — по файлу на папку, рассказ
  в три слоя: заголовок — имя папки, под ним шапка главного файла (**зачем**);
...
порядок рассказа, — остальные файлы по пути; тесты в порядке объявления.
Одинаковые `describe` из нескольких файлов папки (юнит и e2e одной
возможности) сливаются в один раздел.

**Главный файл папки** — `tests/capabilities/<name>/<name>.*`
(`<name>.test.ts`, `<name>.e2e.ts`…), у стандарта — `rule.*`. В нём рассказ
capability; шапка другого файла папки в документацию не идёт — `spec-doc`
называет файл в stderr, `--strict` возвращает код 1.

### Проза — JSDoc

Страница capability — рассказ (правило «Спецификация — тесты» в
`AGENTS.md`): зачем она, что умеет, чем проверено. Названия дают только
«что умеет» и «чем проверено», «зачем» и то, чего не видно из названий, —
проза JSDoc-комментарием `/** … */` в самом тесте; `spec-doc` переносит её в
документацию (отчёты раннеров комментариев не несут — он читает исходники
тем же сканером, что `spec-diff`):

| Где JSDoc | Куда в `docs/spec` |
|---|---|
| в начале главного файла папки, до импортов | абзацы под заголовком capability / стандарта; первый — описание в индексе |
| вплотную перед `describe` | абзац под его заголовком |
...
- Шапка главного файла — для кого capability и какую задачу решает, абзац-два;
  первая фраза — описание в оглавлении. У других файлов папки шапки нет.
...
- `//` и `/* … */` в документацию не идут: причины решений, отвергнутые
  альтернативы, имена файлов, номера issue и устройство теста — для читателя
  кода. Блочные теги (`@see`, `@param`) и всё после первого из них — тоже.
...
- Шапка не в главном файле папки в документацию не идёт (раздел «spec-doc»
  выше): её текст — либо в рассказ главного файла, либо в `//`.
...
«Изменены» — переименования: тест с похожим именем в том же файле и том же
`describe` (похожесть ≥ 0,6) или тест с тем же именем под другим `describe`
той же папки, в том числе из другого файла (так выглядит перекладка юнитов и
e2e в раздел-возможность главного файла); непохожее имя — честно удалён и
добавлен. Пустой список печатается как
